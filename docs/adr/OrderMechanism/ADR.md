...
\- Отправка рыночной стоимости продуктов пользователя. Микросервис подписан на топик получения рыночной стоимости от микросервиса портфеля. Сообщение содержит в ID пользователя и продуктов. Микросервис делает поиск по ID в БД, после чего вычисляет рыночную стоимость и отправляет её портфелю.
