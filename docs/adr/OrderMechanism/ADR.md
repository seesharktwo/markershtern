...
Фоновый процесс для поиска подходящих заявок. Для фонового процесса обработки заявок возможно использовать IHostedService, который будет запускать задачу каждый раз, когда происходит изменение в БД. 
